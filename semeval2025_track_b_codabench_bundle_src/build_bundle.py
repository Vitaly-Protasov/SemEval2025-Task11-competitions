...
def main() -> None:
    ...
    write_zip(
        resources_dir / "track_b_public_data.zip",
        ROOT / "public_data",
        [Path("track_b")],
    )
    write_zip(
        resources_dir / "track_b_reference_data.zip",
        ROOT / "build" / "reference_data",
        [Path(".")],
    )
    write_zip(
        resources_dir / "track_b_scoring_program.zip",
        ROOT / "build" / "scoring_program",
        [Path(".")],
    )
    write_zip(
        resources_dir / "track_b_ingestion_program.zip",
        ROOT / "build" / "ingestion_program",
        [Path(".")],
    )
    write_zip(
        resources_dir / "track_b_zero_submission.zip",
        ROOT / "starting_kit" / "sample_submission",
        [Path("track_b")],
    )
    write_zip(
        ROOT / "starting_kit" / "track_b_zero_submission.zip",
        ROOT / "starting_kit" / "sample_submission",
        [Path("track_b")],
    )
    write_zip(
