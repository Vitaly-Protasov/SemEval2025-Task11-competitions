...
Track C uses macro-F1 per language, matching the shared-task evaluation setup for multilabel emotion detection.

For each submitted language:

- F1 is computed independently for each emotion column.
- The language score is the mean of these per-emotion F1 values.

The set of evaluated emotion columns depends on the language-specific test file. Some languages use five emotion columns, while others use six.
...
- each submitted language must be supported by Track C
...
2. Use `dev` as the labeled source data for cross-lingual experiments.
3. Build prediction files from the released `test` files.
4. Zip the `track_c` folder and upload it.
